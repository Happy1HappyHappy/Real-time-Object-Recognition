...
// Applies the standard cleaning strategy using predefined default parameters.
void MorphologicalFilter::defaultDilationErosion(cv::Mat &src, cv::Mat &dst) {
    customDilationErosion(src, dst, DEFAULT_K_SIZE, DEFAULT_E_STEPS, DEFAULT_D_STEPS);
}
 ...
 */
void MorphologicalFilter::customDilationErosion(cv::Mat &src, cv::Mat &dst, int k_size, int e_steps, int d_steps) {
    ...
    for (int i = 0; i < e_steps; i++) {
        erosion(&current_stage, &next_stage, k_size);
    ...
    for (int i = 0; i < d_steps; i++) {
        dilation(&current_stage, &next_stage, k_size);
        current_stage = next_stage.clone();
    }
...
void MorphologicalFilter::dilation(const cv::Mat* src, cv::Mat* dst, int k_size) {
    int h = src->rows;
    int w = src->cols;
    int pad = k_size / 2;
...
void MorphologicalFilter::erosion(const cv::Mat* src, cv::Mat* dst, int k_size) {
    int h = src->rows;
    int w = src->cols;
    int pad = k_size / 2;
